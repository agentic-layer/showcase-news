...
[source,bash]
----
curl -L https://github.com/cert-manager/cert-manager/releases/latest/download/cert-manager.yaml | kubectl apply -f -
----
...
The Agent Runtime Operator manages AI `Agents`, MCP `ToolServers`, and `ToolRoutes` as Kubernetes custom resources.

See the xref:agent-runtime-operator:agent-runtime:how-to-guides/install.adoc[agent-runtime-operator] installation instructions.
...
1. Install an operator, e.g. the xref:agent-gateway-krakend-operator::how-to-guides/install.adoc[agent-gateway-krakend-operator].
...
1. Install an operator, e.g. the xref:ai-gateway-litellm-operator::how-to-guides/install.adoc[ai-gateway-litellm-operator].
...
[source,bash]
----
helm install news-showcase ./chart
----
...
== Step 5: Automated Agent Evaluation

The showcase includes an automated evaluation pipeline powered by the https://github.com/agentic-layer/testbench[Testbench^] — a Kubernetes-native evaluation system from the Agentic Layer platform. It executes test scenarios against the news-agent via the A2A protocol and scores responses using an LLM-as-a-judge approach with https://docs.ragas.io/[RAGAS^] metrics. The pipeline is orchestrated by https://testkube.io/[Testkube^] and runs automatically whenever the agent is redeployed.
...
* **LLM Judge Model**: `gemini-2.5-flash-lite` — a fast, cost-effective model used to evaluate agent responses via the AI Gateway.
* **Default Threshold**: `0.9` — the minimum score a metric must archive to pass the test.
...
1. **Run** — Sends each scenario's queries to the news-agent via the A2A protocol (`http://news-agent.showcase-news:8000`) and records responses along with OpenTelemetry trace IDs.
2. **Evaluate** — Scores the agent's responses using RAGAS metrics with the configured LLM judge (via the AI Gateway).
3. **Publish** — Exports per-sample evaluation metrics as OpenTelemetry gauges to the OTLP collector, where they can be visualized in Grafana.
4. **Visualize** — Generates a self-contained HTML dashboard with summary cards, score charts, metric distribution histograms, and a searchable results table.
...
For **manual installation (Option B)**, the test workflow resources are already included in the kustomization (`deploy/kustomization.yaml`) and will be applied with `kubectl apply -k deploy/`. However, you must install https://testkube.io/[Testkube^] and the https://github.com/agentic-layer/testbench[Testbench Helm chart^] separately for the workflow to execute.
