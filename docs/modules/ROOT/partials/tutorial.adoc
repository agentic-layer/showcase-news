...
See the xref:agent-gateway-krakend-operator:operator:how-to-guide.adoc[agent-gateway-operator] installation instructions.

===== Install an Agent Gateway instance

Configure an `AgentGateway` instance: xref:agent-gateway-krakend-operator:gateway:how-to-guide.adoc[see example].

==== AI Gateway

===== Install AI Gateway Operator

This operator manages AiGateway resources and manages LiteLLM deployments. It serves as an abstraction layer for LLM provider interactions, providing unified access, security, and intelligent routing.

See the xref:ai-gateway-litellm-operator:operator:how-to-guide.adoc[ai-gateway-litellm-operator] installation instructions.
...
# Agent Gateway
kubectl port-forward -n showcase-news service/my-agent-gateway 8004:8080 &
