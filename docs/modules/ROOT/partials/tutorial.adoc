...
* Install the Agentic Runtime Operator, Agent Gateway Operator, and AI Gateway Operator
...
The Agent Runtime Operator manages AI agents and MCP servers as Kubernetes custom resources. Install the latest version:

[source,bash]
----
curl -L https://github.com/agentic-layer/agent-runtime-operator/releases/download/v0.7.0/install.yaml | kubectl apply -f -
----
...
This operator manages AgentGateway resources and automatically generates KrakenD configurations to expose Agent workloads through a unified gateway interface.

See the xref:agent-runtime-operator:agent-runtime:how-to-guide.adoc[installation instructions]

==== Install an Agent Gateway instance

Configure an `AgentGateway` instance: xref:ai-gateway-litellm-operator:gateway:how-to-guide.adoc[see example].

==== Install AI Gateway Operator

This operator managed AiGateway resources and manages LiteLLM deployments. It serves as an abstraction layer for LLM provider interactions, providing unified access, security, and intelligent routing.

// TODO: Replace instructions with reference after PAAL-195 is done
//See the xref:ai-gateway-operator:operator:how-to-guide.adoc[installation instructions]
...
==== Install AI Gateway Operator instance

// TODO: Replace instructions with reference after PAAL-195 is done
//Configure an `AiGateway` instance: xref:ai-gateway-litellm-operator:gateway:how-to-guide.adoc[see example].

Configure an instance of an `AiGateway`:
...
kubectl delete -f https://github.com/agentic-layer/agent-runtime-operator/releases/download/v0.7.0/install.yaml
